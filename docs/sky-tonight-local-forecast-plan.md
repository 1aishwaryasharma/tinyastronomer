...
- [x] `public/tz-coords.js` generated by `tools/build-tz-coords.ts` from
...
- [x] `resolveLocation()` returns the stored location, else the timezone
      guess. Never touches geolocation.
- [x] `requestDeviceLocation()` wraps `getCurrentPosition` in a promise,
      rounds, stores, and returns `{ lat, lon, source: 'geo' }`. Map the error
      codes to short copy: denied, unavailable, timeout.
- [x] `setManualLocation(lat, lon)` validates and stores.
- [x] `describeLocation(loc)` returns a short label. For the timezone tier use
      the city part of the zone id, for example `Chicago area`. For the other
      tiers format as `41.9°N 87.6°W`.
- [x] Change `public/_headers` Permissions-Policy from `geolocation=()` to
      ...
      changed.
- [x] Confirm `dev-server.ts` passes the header through unchanged.
